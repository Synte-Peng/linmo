...
static int find_event_by_task(uint8_t type,
                              uint16_t task_id,
                              uint32_t p1,
                              uint32_t p2,
                              uint32_t start_index)
{
    ...
    for (uint32_t i = start_index; i < count; i++) {
        ...
        if (event.event_type == type && event.task_id == task_id &&
            event.param1 == p1 && event.param2 == p2)
            return (int) i;
    }
...
static void verifier(void)
{
    ...
    int yield_a = find_event_by_task(EVENT_TASK_YIELD, ids.worker_a, 0U, 0U, 0);
